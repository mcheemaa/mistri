...
# One small cell of the eval against real models: the harness must compact,
# probe, continue, judge, and price a run end to end.
class TestCompactionEvalLive < Minitest::Test
  def setup
    ...
    skip "no OPENAI_API_KEY for the judge" if ENV["OPENAI_API_KEY"].to_s.empty?
  end

  def test_one_cell_runs_end_to_end_on_sonnet_with_astra_judging
    runner = CompactionEval::Runner.new(models: ["claude-sonnet-5"], scenarios: ["ledger_export"],
    ...
    assert_includes 1..5, row.dig(:judge, :resumability)
