...
argument when it equals the expected value after normalization. There is no
judge model, so a run is reproducible and cheap.
...
compaction; `--models all` is the whole catalog. Keys come from the
environment; `.env.development.local` wins over the shell, as in the tests.
...
the comparison in the pull request:

```sh
bundle exec ruby script/compaction_eval.rb run --repeat 2 --folds --out tmp/compaction-eval/candidate.jsonl
