...
# A finance-operations week at Shopify: a payouts ledger export with an exact
# deliverable name, hard rules, numbers that must not drift, a decision
# reversed mid-way, a renamed file, and a credential rule. Every person,
# ticket, repository, and number is invented.
CompactionEval::Scenario.define("ledger_export",
                                summary: "Payouts ledger export with a rename mid-way") do
  filler do |turn, rng|
    shard = turn % 8
    shapes = [
      { ask: "Continue with step #{turn + 1}: check shard #{shard} of the export job and " \
             "report what the log says about retries and latency.",
        doing: "Checking shard #{shard} of the export worker log for step #{turn + 1}; " \
               "looking for retries, skipped jobs and the p95 latency before deciding on " \
               "the batch size.",
        tool: "run_command",
        arguments: { "command" => "tail -n 40 log/payouts_export_worker_#{shard}.log" },
        log: CompactionEval::Filler.worker_log(rng),
        done: "Shard #{shard} looks healthy for step #{turn + 1}: retries are within the " \
              "normal band and latency is stable, so no batch size change is needed yet." },
      { ask: "Step #{turn + 1}: how many payout rows landed in the staging table since the " \
             "last run, and did any of them fail the balance check?",
        doing: "Counting staged payout rows and failed balance checks for step #{turn + 1}.",
        tool: "run_sql",
        arguments: { "sql" => "SELECT status, count(*) FROM payouts_export_staging " \
                              "WHERE run_id = current_run() GROUP BY status" },
        log: CompactionEval::Filler.sql_counts(rng),
        done: "Step #{turn + 1}: the staged counts add up and no row failed the balance " \
              "check, so the run stays green." },
      { ask: "Step #{turn + 1}: grep the reconciliation log for any shop id that appears " \
             "more than once and tell me whether dedupe is holding.",
        doing: "Scanning the reconciliation log for repeated shop ids in step #{turn + 1}.",
        tool: "run_command",
        arguments: { "command" => "grep -c 'duplicate shop_id' log/reconciliation.log" },
        log: CompactionEval::Filler.grep_hits(rng),
        done: "Step #{turn + 1}: no repeated shop id in this window. Reasoning it through: " \
              "the dedupe pass keys on shop id and payout period, and every line in the " \
              "sample carries a distinct pair, so the earlier double counting is not " \
              "recurring under the current query." },
      { ask: "Step #{turn + 1}: confirm the replica is caught up before the next export slice.",
        doing: "Checking replication lag on the read replica for step #{turn + 1}.",
        tool: "run_sql",
        arguments: { "sql" => "SELECT now() - pg_last_xact_replay_timestamp() AS lag" },
        log: CompactionEval::Filler.replica_lag(rng),
  ...
  segment do
    fact :ticket, "FIN-2831",
         at: 0.0,
         text: "We are shipping the Q3 payouts ledger export for ticket %<value>s.",
         probe: "Which ticket id is this work for? Reply with the id only."
    fact :filename, "exports/q3_payouts_ledger_2026-09-05_v7.csv",
         at: 0.0,
    ...
    fact :footer, "Prepared by Nell Adeyemi for Finance (do not distribute)",
         at: 0.0,
    ...
    fact :pr, "https://github.com/Shopify/payouts-ledger/pull/3477",
         at: 0.05,
         text: "The pull request to extend is %<value>s.",
         probe: "Which pull request are we extending? Reply with the URL only."
    fact :decimals, "two",
    ...
    fact :p95, "900",
         ...
         probe: "Above what p95 latency in milliseconds is a shard unhealthy? Reply with the " \
                "number only."
    fact :cutoff, "Friday 17:00 ET",
         at: 0.4,
         text: "Finance needs the file before %<value>s.",
         probe: "What is the delivery cutoff now? Reply with the day and time only."
    fact :error, 'PG::UndefinedColumn: column "merchant_group_id" does not exist',
         at: 0.5, carrier: :tool, answer: 'column "merchant_group_id" does not exist',
         text: "ERROR %<value>s\n  from app/services/payouts_export/rows.rb:41",
         probe: "What exact database error did the export worker log? Quote the error message."
    fact :error_site, "app/services/payouts_export/rows.rb:41",
         at: 0.5, carrier: :assistant,
         text: "The failure comes from %<value>s: the column moved when merchants were grouped, " \
               "so the rows query needs the join through merchant_groups first.",
    ...
    change :filename, "exports/q3_payouts_ledger_2026-09-05_v8.csv",
           at: 0.7, answer: "yes", match: :yes_no,
  ...
  segment do
    fact :infra_ticket, "OPS-2101",
         at: 0.1,
         text: "Infra opened %<value>s for the replica failover; mention it in the handoff.",
         probe: "Which ticket did infra open for the replica failover? Reply with the id only."
    fact :restart, "bin/rails payouts:refresh_views",
         at: 0.2,
    ...
    change :cutoff, "Thursday 15:00 ET",
           at: 0.5,
  ...
  continuation prompt: "Finance approved. Save the export now with the agreed filename, footer, " \
                       ...
                       "values.",
               tool: "save_export",
               description: "Writes the payouts ledger export with the agreed values.",
               schema: lambda {
