...
# An incident session: identifiers from tool output, a rollback rule, a flag
# that must stay off, numbers that change as mitigation lands, and a root
# cause the agent must carry into the postmortem.
CompactionEval::Scenario.define("incident_debugging",
                                summary: "Production incident with a mitigated error rate") do
  filler do |turn, rng|
    pod = "smart-delivery-api-#{%w[6d9f 7b21 9c4e][turn % 3]}-#{%w[x2k9 m7q1 p0d4][turn % 3]}"
    {
      ask: "Step #{turn + 1}: pull the latest logs from #{pod} and tell me whether the error " \
           "signature is still showing up.",
      doing: "Pulling the last 200 lines from #{pod} for step #{turn + 1} and scanning for the " \
             "signature, then checking the error rate on the monitor.",
      tool: "kubectl_logs",
      arguments: { "pod" => pod, "tail" => 200 },
      log: lambda { |chars|
        CompactionEval::Noise.lines(rng, chars) do |_index, random|
          format("%<at>s %<level>s %<route>s status=%<status>d duration=%<ms>dms " \
                 "request_id=req_%<request>08x",
                 at: CompactionEval::Noise.stamp(random),
                 level: %w[INFO INFO INFO WARN][random.rand(4)],
                 route: ["GET /v1/addresses/validate", "POST /v1/deliveries",
                         "GET /healthz"][random.rand(3)],
                 status: [200, 200, 200, 422, 500][random.rand(5)], ms: 8 + random.rand(400),
                 request: random.rand(2**32))
        end
      },
      done: "Step #{turn + 1}: the signature is not in this batch and the monitor is flat, so " \
            "I am moving to the next pod."
    }
  end
  ...
  segment do
    fact :incident, "INC-4471",
         at: 0.0,
         text: "We are on incident %<value>s: smart-delivery-api is failing address " \
               "validation for some recipients.",
         probe: "Which incident id is this? Reply with the id only."
    fact :service, "smart-delivery-api",
         at: 0.0,
    ...
    fact :flag, "smart-delivery-address-type",
         at: 0.05,
         text: "The feature flag %<value>s stays OFF in production until Priya signs off.",
    ...
    fact :error, "NoMethodError: undefined method 'zip' for nil",
         ...
         answer: "undefined method 'zip' for nil",
    ...
    fact :root_cause, "PO box addresses arrive with a nil street line",
         at: 0.5,
         carrier: :assistant, answer: %w[nil PO],
         text: "Root cause: %<value>s, and normalize.rb calls zip on it.",
         probe: "What is the root cause? Answer briefly."
    fact :timeout_env, "EASYPOST_TIMEOUT_MS=2500",
         at: 0.55, carrier: :tool, answer: "2500",
         text: "%<value>s",
         probe: "What is EASYPOST_TIMEOUT_MS set to in the pod? Reply with the number only."
  ...
  segment do
    ...
    fact :revert_mr, "!34981",
         at: 0.7, carrier: :tool,
         text: "Created merge request %<value>s: Revert address type normalization",
         probe: "Which merge request holds the revert? Reply with the MR reference only."
  end

  continuation prompt: "Priya asked for the postmortem stub now. Call file_postmortem once with " \
